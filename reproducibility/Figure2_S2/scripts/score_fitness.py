...
sys.path.append("/path/to/jungle")  # specify path to jungle
import jungle as jg
...
tumor_to_model = pd.read_csv(
    f"{data_directory}/trees/tumor_list.txt", sep="\t",
)

home_dir = "/path/to/savedirectory"
for tumor in tqdm(tumor_list['Tumor'].values):

    fp = f'{data_directory}/trees/{tumor}_tree.pkl'
...
for tree, name in zip(F_empirical.trees, names_trees):
    ...
    print(name)
    outfp = os.path.join(
        home_dir, name, f"mean_fitness.{name}.txt"
    )
