...
sys.path.append("/data/yosef2/users/mattjones/projects/kptc/KPTracer")
from notebooks.utilities import tree_utilities
...
for tumor in tqdm(tumors):
    ...
    if PREPROCESSED:
        ...
        os.path.join(HOME, "trees", tumor, "phylogenetic_distance_matrix.preprocessed.tsv"),
        sep="\t",
        index_col=0,
    )
    else:
        phylogenetic_distance_matrix = pd.read_csv(
            os.path.join(HOME, "trees", tumor, "phylogenetic_distance_matrix.tsv"),
            sep="\t",
            index_col=0,
        )
    edit_distance_matrix = pd.read_csv(
        os.path.join(HOME, "trees", tumor, "edit_distance_matrix.tsv"),
        sep="\t",
        index_col=0,
    )
    ...
    #     os.path.join(
    #         HOME, "trees", tumor, f"evolutionary_coupling.{tumor}.txt"
    #     ),
    #     sep="\t",
    # )
    ...
    edit_zscores_norm.to_csv(
        os.path.join(
            HOME, "trees", tumor, f"evolutionary_coupling.{tumor}.edit_preprocessed.txt"
        ),
        sep="\t",
    )
    ...
    kth_zscores_norm.to_csv(
        os.path.join(
            HOME, "trees", tumor, f"evolutionary_coupling.{tumor}.knn_preprocessed.txt"
        ),
        sep="\t",
    )
